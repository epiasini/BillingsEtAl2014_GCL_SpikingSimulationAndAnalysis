...
GCs were implemented in the spiking network with a conductance-based
integrate-and-fire model whose parameters were set to previously
published experimental averages (Rothman2009; Schwartz2012; see
Supplemental Table 7). Excitatory synaptic conductances were fitted to
paired recording data (Rothman2009; Sargent) with swarm intelligence
techniques (Deb2010). A short term plasticity mechanism (Tsodyks et
al 1998) was implemented for both AMPAr- and NMDAr-mediated
conductances, and voltage-dependent Mg2+ block for the NMDAr-mediated
conductance was modeled as in Schwartz2012. A tonic GABA conductance
was included with experimentally measured amplitude (Rothman2009) and
reversal potential (Seja2012). Simulations were generated with
neuroConstruct (Gleeson2008) and run with NEURON (Carnevale2006) on
UCL's Computer Science Cluster and Legion HPC facility. Cell and
synaptic models are available in NeuroML2/LEMS format (Gleeson2010) on
the Open Source Brain (www.opensourcebrain.org; Gleeson2012);
simulation and data management code is available on GitHub
(github.com/epiasini/network).

**** Analysis of spiking network data
To quantify the transformation performed by the network we measured
input and output population sparseness according to the definition
given in Vinje2000. This is a normalised version of the metric defined
by Treves and Rolls 1991, and is defined as
\[
S = \left(C - \frac{\left(\sum_{i=1}^Cr_i\right)^2}{\sum_{i=1}^C{r_i^2}}\right)\left(C-1\right)^{-1}
\]

where $C$ is the number of cells and $r_i$ is the spike count of cell
$i$. Additionally, because of the abundance of ways in which the
sparseness of a vector can be quantified (Hurley2009), we performed
our analyses with two alternative definitions (the one defined in
Hoyer2004 and 'activity sparseness' as defined in Willmore2001); we
found that this did not affect our results (data not shown). The
/sparsification/ operated by the network on an input stimulus was
defined as the output sparseness divided by the input
sparseness. Mutual information was calculated between the set of N
input patterns and N output network activity classes obtained by
performing a Voronoi (nearest-neighbor) tessellation of the output
space. The N seed points for the tessellation were the centroids of
the clusters obtained by running the k-means algorithm (Lloyd1982,
Pedregosa2009) on a training dataset of 30 repetitions per pattern. To
avoid being trapped in local minima, the algorithm was re-initialised
10 times using ~k_means++~ (Arthur2007), and only the best clustering
was considered in the computation of the centroids. Training data was
not re-used for the computation of MI. Information was calculated with
~pyentropy~ (Ince2009), applying a suitable undersampling bias
correction (Strong1998). An initial 150ms transient was discarded from
all simulations to allow the system to go from resting to steady state
for the pattern being simulated. Independent recordings of the
response to a pattern were extracted from a single simulation by
slicing it in 30ms-long time frames and discarding every other frame,
to allow the system state to decorrelate between the frames we kept
for analysis.

The perfomance of the decoder generally doesn't improve by increasing
M further - reflecting the fact that the tassellation of output space
defined by the decoder matches well with the natural structure of the
data clusters.
