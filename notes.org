...
effect of shortening or lenghtening it. Anyway, it needs to be
...
amplification'), but then again 'ampliication' has other ambiguities,
...
information to guide our intuition as to where to calcolate MI.
**** Input correlations
...
interesting, this case is going to end up in the supplementary
information anyway.

An example of a particularly interesting thing that might happen, by
the way, is if we got a sudden boost to information transmission in
the anatomical model when the spatial scale of the input correlations
matches the typical scale of network connectivity.
...
bottom up with respect to cell and synaptic physiology and tissue
anatomy, as we know from the experiments.

To do this we need:
***** TODO 3D representation of the network with input and output raster plots
