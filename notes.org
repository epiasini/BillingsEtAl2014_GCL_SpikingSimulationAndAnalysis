...
*** TODO finalise a choice for input stimulation rates
...
**** TODO Reintroduce separation between training and testing data sets
...
*** TODO Once I have a sparsity measure, make some exploratory simulations (even with the random graph model for the network) to have a rough idea of the behaviour of the MI and the sparsity for the cases with, say, 4 and 20 dendrites.
    ...
    <2013-12-19 Thu> Figures [[fig:MI_1024]] and [[fig:amp_1024]] show
    information transmission and /amplification/ (analogous to
    P(GC)/P(MF)) *for the tissuel model* with 1024 patterns, 50 trials
    each and active mfs firing at 80Hz. They're incomplete because the
    missing simulations are still running on the cluster.    

    #+CAPTION: Mutual Information for 1024 patterns, 50 trials, 80Hz active mfs.
    #+NAME: fig:MI_1024
    #+ATTR_HTML: :width 60%
    [[file:~/phd/code/network/data/figures/mi.png]]

    #+CAPTION: Amplification for 1024 patterns, 50 trials, 80Hz active mfs. Axes are the same as in figure [[fig:MI_1024]].
    #+NAME: fig:amp_1024
    #+ATTR_HTML: :width 60%
    [[file:amplification.png][file:~/phd/code/network/src/amplification.png]]
