...
*** TODO [4/5] figure 8: results.
    ...
    - [X] figure legend
    - [ ] Example raster plots showing high/low sparsification
...
** Short meeting with Angus and Guy <2014-03-04 Tue>
*** Figure 3
- Delete panel B, and add raster plots from figure 5.
- Possibly add the heatmap for the '*tonic only*' threshold
  scaling. It' s ok to add it here (rather than in figure 4) because
  for it we don't need to explain the DTA; plus it would make it more
  'standalone' and easier to reference later as the configuration that
  we're interested in studying with the spiking network
*** Figure 4
- We want to make sparsification more prominent to show that we're
  interested in the tradeoff between it and information
  transmission. To increase treat the two quantities in a more
  symmetrical fashion, we could *add heatmaps for sparsification*.
- If we don't put it in figure 3, we have to add the new tonic only
  threshold scaling heatmap here.
- How are we going to fit 6/8 heatmaps plus (at least) the small panel
  explaining the DTA in a single figure? Do we need to *take something
  out*?.
*** Figure 5
- Take out the example raster plots (move them to figure 3)
- If we add sparsification heatmaps to figure 4, are the remaining
  panels still needed here?
*** Output sparseness vs sparsification
Probably using sparsification wouldn't have any effect on the
appearance of the plots and it will have the benefit of harmonising
the presentation of the binary and spiking cases.
*** Input-output function for an infinite network
[[file:fig/network_IO_binary.png][file:~/phd/code/network/src/fig/network_IO_binary.png]]
[[file:fig/network_IO_rate_coded.png][file:~/phd/code/network/src/fig/network_IO_rate_coded.png]]
*** Figure 8
Angus proposed to include heatmaps for the correlated input case (here
shown in the mockup for 128 patterns as the 1024 pattern simulations
are still running). Guy suggested we add the network I/O functions
too.

[[file:~/Dropbox/GCLpaper/Billings_etal_Silver_paper_2013/Neuron_Submission/eugenio_figures/figure_8.png]]
*** Sections for the supplementary material (see below).
...
** DONE Sparse encodable range vs integration window length
   CLOSED: [2014-02-23 Sun 18:35]
** DONE Robustness to changes in 'on' firing rate: run simulations for 40Hz and 120Hz.
   CLOSED: [2014-02-11 Tue 14:09]
** TODO Effect of DTA
** TODO Anatomical vs random model
** TODO Excitatory vs inhibitory scaling
** TODO Relationship between p(MF) and input sparseness (goodness of sparseness measure)
** TODO Why is the output sparseness different from 0.5 for the network with 1 dendrite?
*** Relationship between p(MF) and sparseness (or: binary and rate-coded activity sparseness measures)
The activity sparseness measure doesn't directly map to p(GC) in the
binary case: indeed if we measure the
[[file:fig/spasity_measure.png][file:fig/spasity_measure.png]]
* Other ideas
...
* Drafts for paragraphs to insert in the paper
*** Methods
    #+NAME:figure_7
    #+CAPTION:Figure 7 in the paper. Estimation of input/output mapping in a spiking network model of the GCL. *A*: Example of synaptic input to a granule cell model with four dendrites. Top two traces: excitatory (AMPA, NMDA) synaptic conductances from active MF terminals. Middle traces: excitatory synaptic conductances from inactive MF terminals. Bottom trace: tonic inhibitory (GABA) conductance. *B*: Synaptic integration. Inset: rate-coded input-output curve for the GC model with 4 synaptic inputs generated from independent Poisson spike trains. *C*: Membrane potential of the model resulting from injection of conductances in panel A. *D*: Fit of the short-term plasticity model (red trace) of the AMPA component of the MF to GrC synapse to an experimental recording of a 100Hz conductance train (grey trace). *E*: same as *D* but for the NMDA component and a 80Hz conductance train. *F*, *G*, *H*: Network simulation set-up. *F*: A binary stimulus pattern (black and white barcode) is selected out of a set of N patterns. A Poisson spike train is generated for each mossy fibre terminal (red raster plot) and played into the network. Red barcode: empirical spike counts for the given realisation of the spike trains (analogue to one column of the red binary input matrices in figure 4). *H*: Three-dimensional view of the network model. Mossy fibre terminals are depicted in red and granule cells in blue. *G*: granule cell firing activity in response to the selected input pattern. Blue barcode: granule cell spike count vector, used for estimation of output sparseness and MI. The output spike count vector is assigned to one of K output classes by finding the closest centroid in a codebook prepared by running the k-means algorithm on a separate training data set. Mutual information is then estimated between the N input patterns and the K output classes, for K=N.
    #+ATTR_HTML: :width 800px
    [[file:fig/figure_7.svg]]
...
was varied (see Supplemental Information for additional discussion).

All models we used are available in NeuroML/LEMS format (Gleeson2010)
on the Open Source Brain.
**** Simulation set-up for the spiking network
...
the uniform binary network we simulated the response of the system to
a set of 1024 random rate-coded patterns. Mirroring the binary case,
each pattern was determined by selecting a random subset of mossy
fibre terminals and designating them to be 'active' (black lines in
the black and white barcode in figure 7E; top two traces in figure
7A). During the simulation, active mossy fibre terminals fired Poisson
spike trains with mean rate 80Hz, while inactive ones fired with a
background rate of 10Hz (red raster plot in figure 7). These rates
were selected to be representative of typical rate-coded inputs to the
cerebellum (Van Kan 1993); but see the Supplemental information for
further discussion. The spiking activity of the network was recorded
over a 30ms time window (*shall we add* /disregarding an initial
transient to allow the system to start from an appropriately
randomized state?/).

**** Analysis of spiking network simulations
We consider the output of a simulation to be the vector of all granule
cell spike counts (blue barcode in figure 7F). Because of the
randomness in the generation of the input spike trains, any given
input pattern gives rise to a distribution of possible outputs. In
principle, calculating the mutual information between input and output
requires the estimation of this distribution for each input pattern
(together with a choice of prior distribution over input patterns,
which we take to be uniform). In practice, this is unfeasible due to
the very large number of simulations required (a form of /curse of
dimensionality/ (Quian Quiroga 2009)). To simplify the task, we reduce
the dimensionality of the output space by cascading the network with a
/decoder/ which labels output spike count vectors as belonging to one
out of a fixed number of classes M, such that the communication
channel represented by the network plus the decoder maps N input
patterns to M output classes, and for a resonable value of M this
(toghether with an appropriate undersampling bias correction
technique; see Treves1995, Ince2009) allows for direct estimation of
the MI.

The decoder is a simple euclidean nearest neighbor classifier, or a
Voronoi tassellation of the output space. The M points defining the
tassellation are the centroids of the clusters obtained by running the
k-means algorithm (Lloyd1982, Pedregosa2009) on a dataset generated by
simulating each input pattern 30 times (see supplemental
information). This dataset is then discarded and it is not used for
calculation of the MI. For our analyses we set M=N, as this is the
lowest number of patterns that allows full recovery of the
information, and as the perfomance of the decoder generally doesn't
improve by increasing M further - reflecting the fact that the
tassellation of output space defined by the decoder matches well with
the natural structure of the data clusters.

To measure the sparseness of a population of spiking cells we use the
/activity sparseness/ quantity introduced in Willmore2001. This
corresponds to calculating the standard deviation of the number of
spikes fired by the cells across the population and computing the
fraction of cells firing fewer spikes than that. The /sparsification/
operated by the network on an input stimulus is then defined as the
output sparseness divided by the input sparseness.

*** Results
      #+NAME:figure_8
      #+CAPTION:Figure 8 in the paper. Input-output transformation for the spiking network, calculated with a set of 1024 stimulus patterns. *A*: 3D view of a sample random stimulation pattern. Red spheres are active MF terminals, white ones are inactive. *B*: Mutual information for the spiking network cascaded with the decoder for 1024 uncorrelated stimulation patterns. Compare to figure 4. *C*: 1 - population activity sparseness. *D*, *E*, *F*: same as above but for a set of 1024 spatially correlated patterns, where MF terminals are activated in groups of 5. *G*: average spikes per GC vs average spikes per MF across all patterns and all values of p(MF), parametrised by the number of synaptic connections, for independent and spatially correlated inputs. *H*: Average MI vs average sparsification (with averages taken across all values of p(MF)) parametrised by the number of synaptic connections, for independent and spatially correlated inputs. 
      #+ATTR_HTML: :width 800px
    [[file:fig/figure_8.svg]]
...
analytical approach, we built a spiking implementation of the
anatomical network model where cell and synaptic dynamics were tightly
constrained by experimental measurements. To quantify the input/output
transformations operated by the network, we simulated 1024 rate-coded
input patterns for several combinations of ~d~ and p(MF). Because of
the richer nature of the inputs and outputs compared to the binary
case, and because of the noise introduced in the input/output
transformations by the stochasticity of the input spike trains, we had
to generalize our measure of sparseness and to measure the full
Shannon information rather than just the output entropy (see Methods).

The estimated MI and sparsification (illustrated in figure 8 for a
number of settings of ~d~ and ~p(MF)~) match very well with the
analogous quantities for the binary model in presence of tonic
inhibition (figure 4). In particular, networks with a small number of
dendrites are optimal for wide range sparse encoding, while network
with many dendrites have a sparser output at the expense of
transmitted information.

*paragraph on correlated inputs*
*** Supplementary Information
#+NAME:supplementary_7
#+CAPTION:Supplementary figure 7 in the paper. *A*: rate-coded input-output curve for the GC model with a variable number of synaptic inptus. *B*: autocorrelation of the GC model output (with 4 synaptic inputs) for 2, 3 or 4 active inputs (the case of 1 active input is not shown as the resulting firing rate is very low). Red line: constant value of the autocorrelation of a Poisson point process with the same mean intensity. Green area: 30ms time window. *C*: Mutual information estimated for different lengths of the time window over which network activity is recorded. *D* Mutual information calculated, for a specific network and p(MF) setting, as the size of the datasets used to train the decoder (top, black line) and to perform the estimation (bottom) is varied. This is a stringent test for the performance of the decoder, as the network performs poorly for the chosen value of ~d~ and p(MF), transmitting only a fraction of the information present in the input. Red, green and blue lines in the top plot: MI estimated with (respectively) no undersampling bias correction, Nemenmann-Shafee-Bialek correction and Panzeri-Treves correction.
[[file:fig/supplementary_7.svg]]

#+NAME:supplementary_8
#+CAPTION:Supplementary figure 8 in the paper. *A*: Mutual information for 128 patterns for different NADT settings. In these simulations, an extra amount of GABA conductance, proportional to p(MF), is introduced. The NADT parameter is defined as a scaling coefficient for this proportionality relation. For example, for NADT=1 and p(MF)=0.6 the GABA conductance will be increased by 60%, whereas for the same p(MF) but NADT=0.3 the increase will be of 20%. NADT=0.3 is a realistic estimate of how the time-averaged GABA conductance contributed by Golgi cells could vary with network activity (Ward and Piasini, unpublished observations.) *B*: average spikes per GC vs average spikes per MF across all patterns and all values of p(MF), parametrised by the number of synaptic connections. *C*: Average MI vs average sparsification (with averages taken across all values of p(MF)) parametrised by the number of synaptic connections. Note how the 'realistic' NADT=0.3 setting increases performance for networks with few synaptic connections. *D*, *E*, *F*: same plots as above but exploring the effect of a different choice for the firing rate of the active mossy fibres. At shown at the bottom of panel *E*, there exists a threshold level of MF activity above which the relationship between number of synaptic connections and GC activity is reversed, and the most active networks are those with more connections. This is reflected in panel *F*, as the netwoks with many synaptic connections lose the sparsification advantage they have when the inputs are encoded at lower rates, collapsing the Pareto front and leaving those with fewer connections as an optimal choice for lossless sparse encoding, even though the maximum sparsification attainable is very low.

Note that when the firing rate is high the networks with many synaptic connections lose their sparsification advantage. This is because, as shown
[[file:fig/supplementary_8.svg]]
**** Effect of spatial correlations in input patterns
     #+NAME:input_pattern_uncorrelated
     #+CAPTION:Uncorrelated input pattern for p(MF)=0.2 (active mf terminals in red, inactive terminals in black)
     #+ATTR_HTML: :width 400px
     [[file:fig/input_pattern_iscs0_mf0.2.png][file:fig/input_pattern_iscs0_mf0.2.png]]
     #+NAME:input_pattern_correlated
     #+CAPTION:Spatially correlated (domain size 5) input pattern for p(MF)=0.2 (active mf terminals in red, inactive terminals in black)
     #+ATTR_HTML: :width 400px
     [[file:fig/input_pattern_iscs4_mf0.2.png][file:fig/input_pattern_iscs4_mf0.2.png]]
   ...
   number of dendrites.
   #+END_QUOTE
**** Effect of choice of stimulation rates
...
**** Effect of time window used for estimating the MI
   ...
   #+NAME: fig:adur_15
   #+CAPTION: MI heatmap for 128 patterns calculated over a 15ms time window.
   [[file:fig/mi_adur_15.png][file:fig/mi_adur_15.png]]
   #+NAME: fig:adur_45
   #+CAPTION: MI heatmap for 128 patterns calculated over a 45ms time window.
   [[file:fig/mi_adur_45.png][file:fig/mi_adur_45.png]]
   #+NAME: fig:adur_60
   #+CAPTION: MI heatmap for 128 patterns calculated over a 60ms time window.
   [[file:fig/mi_adur_60.png][file:fig/mi_adur_60.png]]
**** Effect of number of trials used to train the decoder
...
**** Effect of number of trials used to estimate the information (undersampling bias)
...
***** Sparsification
Generally speaking, the input/output relationship for the network
becomes steeper as ~d~ increases. This sentence can be given precise
meaning in several ways, depending on whether we're looking at the
binary or the rate-coded case and (at least in the rate-coded case)
what we exactly mean by input and output.
****** Binary case
Even if the \phi/d ratio is kept fixed, the probability for a cell to
be above threshold becomes a steeper function of p(MF).

***** Single-cell effects
***** Network effects
* Notes on the convergence between my part and Guy's <2014-03-11 Tue>
*** figure 1
do I have to provide a 3D visualisation of the network with the
correct red/blue color scheme? I can't provide it as vector
graphics, though.
*** figure 2
invert N..1 to 1..N pattern numbering (top to bottom)?
*** figure 3
I thought this was going to be just the same as the old figure 2, plus
the network activity examples from figure 5 (only with fixed threshold
setting to be able to introduce them before the NADT is mentioned).
*** figure 4
This is still missing at the moment. It should have:
- the heatmaps (information and p(GC)) for the 'fixed threshold which
  scales with the connectivity' case. I think Angus wanted to do away
  with the results for the pure fixed threshold setting, but it's best
  to check with him.
- as an aside, why not use the same colormap for p(GC) as we are
  doing for the information? And is it possible to use a continuous
  colorbar instead of the discrete one that's there at the moment?
  the last thing is not too iportant.. one option could also be to
  get the colorbar from figure 8.
- A plot of p(GC) vs p(MF) as the synaptic connectivity changes in
  that same configuration. I propose to use the same colormap I use in
  figure 8 to indicate the number of dendrites.
*** figure 5
This should contain what is in figure 4 at the moment, except for the
plot with DTA+tnic which should be removed. The existing figure 5
should be split in two: the top part goes in figure 3 or figure 4,
and the bottom part should be discarded or moved to the SI.
*** figure 6
Do the results change if we plot the average output entropy instead
of the encodable range? the encodable range is not a difficult
concept, but using it in the plots requires the reader to remember
the definition and actively think about its meaning. 'Average
information' is perhaps more self-explanatory (even though it may be
possible to misinterpret it as the information across patterns with
different p(MF)?) and - because it doesn't do any thresholding - has
a smoother dependence on the overall performance of the
network. Finally, it's the measure I use in the spiking case.
