...
*** Results
      #+NAME:figure_8
      #+CAPTION:Figure 8 in the paper. Input-output transformation for the spiking network with tonic inhibition scaling as the number of dendrites, calculated with a set of 1024 stimulus patterns. *A*: Mutual information for the spiking network cascaded with the decoder. Compare to figure 4. *B*: sparsification . *C*: efficient frontier plot showing how the number of dendrites controls the tradeoff between MI (averaged over all values of p(MF) for a given number of dendrites) and sparsification.
      #+ATTR_HTML: :width 800px
