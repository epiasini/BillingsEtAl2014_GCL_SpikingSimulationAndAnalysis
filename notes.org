...
**** TODO Use ~bias~ parameter to control tonic GABA.
     ...
     simulations, though.
**** TODO Implement DTA
**** TODO Implement simple sinusoidally varying input
...
**** DONE Enable reuse of larger dataset whenever they are available from previous simulation runs/parameter sweeps.
     ...
     existing archive.
**** TODO Don't simulate a given pattern if its corresponding tar archive is already on disk
**** TODO Make sure all tar single-pattern archives are present before starting compression step
...
*** TODO [0/5] figure 7: presentation of the model.
    ...
    - [ ] f/f curve for cell model with (optionally) experimental data
      for comparison
    - [ ] example voltage trace
    - [ ] example of synaptic dynamics with fit to experimental data
    - [ ] 3D visualisation of network structure with input and output
      raster plots
    - [ ] cartoon explanation of MI and data analysis (spike counting
      and clustering)
*** TODO [0/2] figure 8: results. 
    This should contain:
    - [ ] 'raw data' in the form of a MI heatmap for 1024 patterns, to
      show qualitative match to figure 3A in the simplest possible
      case (with fixed inhibition). We should either spend some words
      to address the fact that 1024 is a very small number compared to
      1 billion patterns, by claiming that increasing the number of
      patterns should only make the reduction of encodable range
      stronger, or show this result alongside a version of figure 3A
      opportunely scaled down to 1024 patterns.
    - [ ] global results in form of an 'efficient frontier' kind of
      ...
      feasible).
*** TODO Mockup of figures 7 and 8
...
*** TODO Proof-of-principle figure with sinusoidally varying input rate
    ...
    oscillate at some carrier frequency (10Hz? it might be too slow
    for our short time window). Background activity from 'off' fibers
...
** TODO Fix crash that happens when trying to analyse more than about 1024 (patterns) * 70 (trials) observations
