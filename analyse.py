...
for k,par_combination in enumerate(parameter_space):
    ...
    if plotting_mode == 'precision':
        ax.plot(decoder_precision, tr_direct_mi, label='n_trials%d, train%d, direct' % (n_trials, training_size), linestyle='-.', color=color)
        ax.plot(decoder_precision, ts_decoded_mi_plugin, label='n_trials%d, train%d, plugin' % (n_trials, training_size), color=color)
