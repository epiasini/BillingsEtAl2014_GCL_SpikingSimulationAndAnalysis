...
for k,par_combination in enumerate(parameter_space):
    ...
    if plotting_mode == 'precision':
        ax.semilogx(decoder_precision, tr_direct_mi, linestyle='-.', color=color)
        ax.semilogx(decoder_precision, ts_decoded_mi_qe, label=r'mf act. prob: %.1f' % (active_mf_fraction), color=color)
...
i_at_npatt_plot = ax2.imshow(info_at_npatterns, interpolation='none', cmap='coolwarm', origin='lower')
...
ax2.set_yticks(np.arange(len(active_mf_fraction_range)))
